...
- Active integration base: `origin/codex/v0.0.7`. Fetch its latest tip before
  creating a new task worktree. Update this record and the root agent instructions
  whenever the user advances the active SemVer branch.
- Before building deliverable artifacts, assign a branch-specific SemVer prerelease
  and synchronize binary/package/image versions. Record branch and commit and
  verify the built binary version; do not relabel existing binaries.
