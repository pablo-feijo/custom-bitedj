...
[Agent tooling index](README.md). Required before making changes.

## Start or resume

1. Read [the active release record](../BRANCH_VERSIONING.md). It owns the active
...
3. Create a separate `codex/<topic>` branch/worktree from the freshly fetched
   active remote tip, following [branch creation](../BRANCH_VERSIONING.md#starting-work).
...
5. Select relevant guides from the [tooling index](README.md). Read architecture
   before native/UI/controller edits and the UI workflow before visible changes.
...
- Prefer repository scripts and purpose-built APIs/CLIs over improvised shell
  sequences or GUI clicks. Follow [build/test tools](build-and-test.md) before
...
- Keep scratch helpers, execution notes and generated output in ignored task
  runtime directories. Follow [repository layout](../REPOSITORY_LAYOUT.md) for
  reusable scripts, fixtures and docs; do not accumulate files at the root.

## Validate and finish

- Pick the relevant [test layer](../TESTING.md#run-the-appropriate-layer).
  Documentation-only edits need link/anchor and diff checks. Asset reuse must
  follow the [compiled-output cache protocol](../TESTING.md#reuse-compiled-outputs-for-asset-changes)
  and still rerun applicable tests. New deliverables need rebuilt, verified
  [branch-specific versions and provenance](../BRANCH_VERSIONING.md#every-work-branch-has-an-identifiable-binary).
...
- Before committing, review the diff, run `git diff --check`, verify configured
  ...
  after staging. Follow [storage policy](../GIT_STORAGE.md): 1 MiB staged blob
...
- After every authorized merge and push, record the exact published SHA and
  run links, then follow [post-merge CI and repair](../TESTING.md#post-merge-ci-check-and-repair)
...
- Finish authorized integration with the [cleanup audit](cleanup.md), including
