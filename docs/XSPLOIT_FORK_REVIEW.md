...
| --- | --- | --- | --- |
| DOC-01 | README identity, upstream links, historical description, NOTICE | Previous README mainly described the base project | **Selected, small:** documentation prepared on this branch; review wording and links |
| FX-01 | Programmatic effect enable publication, commit `4c1dfec590`; `src/effects/effectslot.cpp` | Our `EffectSlot::setEnabled` only sets the control; theirs explicitly publishes engine state | **Investigate first, small:** reproduce on our call paths, verify enable/disable and unchanged-state behavior before adopting |
| LIB-01 | Column order, saved text size, selected-track preservation when sorting; `src/library/librarycolumncontrol.cpp`, `src/widget/wtracktableview.cpp` | We already expose column widths/visibility; touch drag is custom | **Investigate first, medium:** separate each improvement; verify 1024×600, persistence, sorting, and drag-to-deck |
...
| RB-01 | Safer Device Library reading: full page indices, cycles/bounds, DAT-only beat/cue passes; `src/library/rekordbox/rekordboxpagechain.h`, `rekordboxfeature.cpp` | Existing reader and per-USB overrides are shared foundations | **Investigate first, medium:** malformed and real exports, DAT-only, corrupt EXT, repeated analysis paths, unplug/replug; audio load must survive optional analysis failure |
...
This documentation branch retains version **0.0.6**. Checked: `BITEDJ_VERSION`
