...
void allshader::WaveformRenderMark::drawPhrases(const QMatrix4x4& matrix) {
    ...
    if (!mixxx::showWaveformPhrases() || !track || m_isSlipRenderer || track->getSampleRate() <= 0) return;
    ...
    const int height = std::max(1, int(std::min(10 * scaleFactor(), double(breadth / 3))));
    ...
    const double duration = m_waveformRenderer->getTrackSamples() /
            (2.0 * double(track->getSampleRate()));
    const double first = m_waveformRenderer->getFirstDisplayedPosition() * duration;
    const double last = m_waveformRenderer->getLastDisplayedPosition() * duration;
    ...
    for (const auto& label : labels) {
        ...
        if (!texture) {
            QFont font; font.setPixelSize(std::max(1, height - 2));
