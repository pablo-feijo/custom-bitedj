...
if [[ -z "${BITEDJ_TEST_INSTANCE:-}" && -f "${SCRIPT_DIR}/test-config/active-instance" ]]; then
    BITEDJ_TEST_INSTANCE="$(cat "${SCRIPT_DIR}/test-config/active-instance")"
fi
if [[ -z "${BITEDJ_TEST_INSTANCE:-}" ]]; then
    WORKTREE_ID="$(printf '%s' "${SCRIPT_DIR}" | cksum | awk '{print $1}')"
...
RESULTS_DIR="${SCRIPT_DIR}/test-results/${CONTAINER_NAME}"
CONFIG_DIR="${SCRIPT_DIR}/test-config/${CONTAINER_NAME}"
...
verify_test_instance_owner() {
    ...
    if [[ "${owner}" != "${SCRIPT_DIR}" || "${branch}" != "${current_branch}" ]]; then
