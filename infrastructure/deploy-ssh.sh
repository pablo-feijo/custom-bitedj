...
expect -c "
set timeout 30
spawn ssh -o StrictHostKeyChecking=no pi@${PI_IP} \"echo ${PASSWORD} | sudo -S mv /tmp/bitedj /usr/bin/bitedj && echo ${PASSWORD} | sudo -S chmod +x /usr/bin/bitedj && echo ${PASSWORD} | sudo -S rm -rf /usr/share/mixxx && echo ${PASSWORD} | sudo -S mv /tmp/mixxx_share /usr/share/mixxx && echo ${PASSWORD} | sudo -S sh -c 'grep -q WLR_NO_HARDWARE_CURSORS /etc/environment || echo WLR_NO_HARDWARE_CURSORS=1 >> /etc/environment' && echo ${PASSWORD} | sudo -S systemctl restart lightdm\"
expect \"*?assword:*\"
send \"${PASSWORD}\r\"
expect eof
"
