...
## Active SemVer Base and Artifact Versions
- The active integration branch is `codex/v0.0.7` (remote: `origin`). Start every new task from its freshly fetched tip in a separate `codex/<topic>` worktree; never default to an older release or the current checkout.
- When the user advances the active SemVer branch, update this record and `docs/AGENTS.md` together. Track the base branch, resolved commit, feature branch and merge target in each task checklist. Reuse the task worktree for follow-ups.
- Before building deliverable artifacts, assign a valid SemVer for that branch and keep binary, package and image versions consistent. Development artifacts need a distinct prerelease identifier; record branch and commit as provenance. Verify the binary version before delivery; never relabel an old binary.


## Artifact Versioning
- Before building deliverable artifacts on any branch, update that branch's BiteDJ product version using SemVer (`MAJOR.MINOR.PATCH`, with an optional prerelease suffix for development builds). Keep the upstream Mixxx base version independent.
- Choose the version for the branch and change scope; do not copy a version from another branch or silently reuse a released version for changed artifacts. Use patch increments for fixes and minor increments for new features, unless the branch has an explicitly agreed release target.
- Keep `BITEDJ_VERSION` / `BITEDJ_VERSION_PRERELEASE`, package metadata, archive names, and OS image names consistent with the artifact's product version. Include the source branch and commit in build provenance so artifacts remain traceable.
- Verify the built binary's reported version and packaged version before delivery. Do not relabel an existing binary as a newer version without rebuilding it.
